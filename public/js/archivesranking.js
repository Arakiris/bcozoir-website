...
window.createRankingArchives = function (lenghtlast, previousYear, url, url_storage) {
    ...
    var renderRanking = function renderRanking(podia, parent) {
        var dateString = podia[0].date.substring(0, 10).split('-');

        var markup = '\n            <div class="archives__tables archives__tables-ranking" id="table-' + dateString[0] + '"> \n        ';

        podia.forEach(function (el) {
    ...
    var renderLineRanking = function renderLineRanking(podium, parent) {
        var dateString = podium.date.substring(0, 10).split('-');
        ...
        // Render first column
        if ((typeof podium.tournament.rules_url == "undefined" || podium.tournament.rules_url == null) && podium.tournament.rules_url == 0 || (typeof podium.tournament.rules_url == "undefined" || podium.tournament.rules_url == null) && podium.tournament.is_rules_pdf == 1) {
        ...
        if (typeof podium.tournament.members !== "undefined" && podium.tournament.members.length > 0 || typeof podium.tournament.teams !== "undefined" && podium.tournament.teams.length > 0) {
            ...
            if (podium.tournament.formation == 0 && typeof podium.tournament.members !== "undefined" && podium.tournament.members.length > 0) {
                ...
                podium.tournament.members.forEach(function (member) {
                    if (member.pivot.order_display) markup += renderMemberPodiaRanking(member);
                });
                markup += '</div>';
            }

            if (podium.tournament.formation == 1 && typeof podium.tournament.teams !== "undefined" && podium.tournament.teams.length > 0) {
                podium.tournament.teams.forEach(function (team) {
            ...
            if (podium.tournament.formation == 0 && typeof podium.tournament.members !== "undefined" && podium.tournament.members.length > 0) {
                podium.tournament.members.forEach(function (member) {
                    if (member.pivot.order_display && member.pivot.rank) markup += '<div class="event__rank-display"> ' + member.pivot.rank + ' </div>';
                });
            }

            if (podium.tournament.formation == 1 && typeof podium.tournament.teams !== "undefined" && podium.tournament.teams.length > 0) {
                podium.tournament.teams.forEach(function (team) {
                    if (team.order_display && team.rank) markup += '<div class="event__rank-display"> ' + team.rank + ' </div>';
                });
            }
        }
        markup += '\n            </div>\n        ';
    ...
    var renderMemberPodiaRanking = function renderMemberPodiaRanking(member) {
    ...
    var renderTeamPodiaRanking = function renderTeamPodiaRanking(team) {
