...
window.createTournamentArchives = function (lenghtlast, previousYear, url, url_home, url_image, url_storage) {
    var getDataAjax = function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/__WEBPACK_IMPORTED_MODULE_0_babel_runtime_regenerator___default.a.mark(function _callee(url, id, token) {
        ...
        return function getDataAjax(_x, _x2, _x3) {
    ...
    if (previousYear !== null) {
        var bottom_pag = document.querySelector('.event__bottom');
        ...
        for (var i = 0; i < tabs.length; i++) {
            tabs[i].addEventListener('click', function (event) {
                ...
                var parentToAddElement = document.getElementById(tab_id);
                var id = tab_id.substring(4);
    ...
    function getDataTournament(id, token, parent, url) {
        ...
        getDataAjax(url, id, token).then(function (tournaments) {
    ...
    var renderTournament = function renderTournament(tournament, parent) {
        var dateString = tournament[0].start_season.substring(0, 10).split('-');

        var markup = '\n            <div class="archives__content archives__paginate" id="paginate-' + dateString[0] + '">\n                <div class="archives__tables" id="table-' + dateString[0] + '"> \n        ';

        tournament.forEach(function (el) {
    ...
    var renderLineTournament = function renderLineTournament(tournament, parent) {
        ...
        var date = new Date(dateString[0], parseInt(dateString[1]) - 1, dateString[2]);

        var options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
        var dateFormat = date.toLocaleString('fr-FR', options);
        dateFormat = dateFormat.charAt(0).toUpperCase() + dateFormat.slice(1);

        var storage = url_storage;

        // Render first column
        var markup = '\n            <div class="archives__row">\n                <div class="event__single-information">\n        ';

        if (typeof tournament.rules_url != "undefined" && tournament.rules_url != null && tournament.is_rules_pdf == 0 || typeof tournament.rules_pdf != "undefined" && tournament.rules_pdf != null && tournament.is_rules_pdf == 1) {
        ...
        if (typeof tournament.pictures_count !== "undefined" && tournament.pictures_count > 0) markup += activeURL(url_home + '/tournoi/' + tournament.slug + '/photos', url_image + '/tournament/tournament-pictures.png', false);else markup += inactiveURL(url_image + '/tournament/tournament-pictures.png');

        if (typeof tournament.videos_count !== "undefined" && tournament.videos_count > 0) markup += activeURL(url_home + '/tournoi/' + tournament.slug + '/videos', url_image + '/tournament/Tournament-videos.png', false);else markup += inactiveURL(url_image + '/tournament/Tournament-videos.png');
    ...
    var renderMemberTournament = function renderMemberTournament(member) {
        var markup = '<div class="event__team-line">';
    ...
    var renderTeamTournament = function renderTeamTournament(team) {
    ...
    var renderMember = function renderMember(member) {
        var storage = url_storage;
        var age = null;
    ...
    var rendercolumnURL = function rendercolumnURL(item, url, img, target) {
    ...
    var activeURL = function activeURL(url, img, target) {
    ...
    var inactiveURL = function inactiveURL(img) {
        return '\n            <div class="event__single-image event__single-image--disable">\n                <div class="event__cell--disable">\n                    <img class="event__single-logo occasion-image-logo" src="' + img + '" alt="Lien lexer du r\xE9sultat">\n                </div>\n            </div>\n        ';
    };
};
