                                                    ...
                                                    @if (!is_null($team->order_display))
                                                        ...
                                                        @foreach ($team->members as $member)
                                                            ...
                                                            @else
                                                                /
                                                            @endif
                                                            <div class="event__tooltip event__tooltip-ranking event__tooltip-team-rank inline-block {{ ($member->is_licensee == 'Licencié') ?  'event__tooltip-licensee' : 'event__tooltip-adherent' }}">
                                                                <p class="event__ranking-paragraph {{ ($member->club_id != 1) ? 'otherClub' : '' }}">{{ $member->last_name }} {{ $member->first_name }}</p>
                                                                <div class="event__tooltip-event event__tooltip-event-ranking {{ ($member->is_licensee == 'Licencié') ?  'event__tooltip-event-licensee' : 'event__tooltip-event-adherent' }}">
                                                                    <img class="event__tooltip-img" src="{{ ($member->picture->first()) ? asset('storage' . $member->picture->first()->path) : null }}" alt="Photo de {{ $member->last_name }} - {{ $member->first_name }}">
                                                                    <div class="event__tooltip-content">
                                        ...
                                        @if ($podium->tournament->formation == 1 && isset($podium->tournament->teams) && $podium->tournament->teams->count()>0)
                                            ...
                                            @foreach($podium->tournament->teams as $team)
                                                @if (!is_null($team->rank))
                                                    <div class="event__rank-display" id="rank-{{$i}}">{{$team->rank}}</div>
                                                    <?php $i++ ?>
                                                @endif
                                            @endforeach
                                        @endif
...
@section('scripts')
    ...
    <script>
        ...
        createRankingArchives(lenghtlast, previousYear, url, url_storage);
    </script>
@endsection